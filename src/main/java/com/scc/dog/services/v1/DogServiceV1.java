...
public class DogServiceV1 {
   ...
   public DogObject getDogById(int dogId) {
      ...
      try {
         ...
         if (dog == null) {
            return (DogObject) new DogObject().withId(0);
         } else {
            return (DogObject) new DogObject().withId(dog.getId()).withNom(dog.getNom()).withSexe(dog.getSexe())
                  .withDateNaissance(formatDtNaissance(dog.getDateNaissance()))
                  .withLof(searchFrenchPedigree(dog.getId())).withTatouage(dog.getTatouage())
   ...
   private ResponseObjectList<DogObject> buildFallbackDogList(String token) {
      ...
      list.add((DogObject) new DogObject().withId(0));
