...
public class TitleService {
   ...
   public List<Title> findByObtentionDateGreaterThanEqual(String referenceDate) {
      ...
      logger.debug("In the titleService.findByObtentionDateAfter call, trace id: {}",
   ...
   public void save(Title syncTitle, Long timestamp) {
      ...
      try {
         ...
         if (title == null) {
            ...
            syncTitle.withTimestamp(new Timestamp(timestamp));
            ...
            if (title.getTimestamp().getTime() < timestamp) {
               ...
               title.withId(syncTitle.getId()).withIdDog(syncTitle.getIdDog()).withIdTitle(syncTitle.getIdTitle())
                     .withTitle(syncTitle.getTitle()).withName(syncTitle.getName()).withType(syncTitle.getType())
                     .withCountry(syncTitle.getCountry()).withObtentionDate(syncTitle.getObtentionDate())
