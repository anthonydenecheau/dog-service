...
import com.scc.dog.repository.ParentRepository;
...
public class ParentService {
   ...
   public Parent getParentById(int id) {
      ...
      logger.debug("In the parentService.getParentByIdDog() call, trace id: {}",
   ...
   public void save(Parent syncParent, Long timestamp) {
      ...
      try {
         ...
         if (parent == null) {
            ...
            syncParent.withTimestamp(new Timestamp(timestamp));
            ...
            if (parent.getTimestamp().getTime() < timestamp) {
               ...
               parent.withId(syncParent.getId()).withName(syncParent.getName()).withAffixe(syncParent.getAffixe())
                     .withOnSuffixe(syncParent.getOnSuffixe()).withTimestamp(new Timestamp(timestamp));
